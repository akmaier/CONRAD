                        ...
                        )
{
    ...
    for ( uint i = 0; i < n_per_work_item; i++, lcl_off += LOCAL_GROUP_XDIM)
    {
        ...
        bool in_range = ( lcl_off < n && mask[u*n0 + proj]);
		...
		float2 value = (float2)(0,0);
		uint offSetAPreCalc = mul24(proj,n0);
		uint offSetBPreCalc = mad24(v,n01,mul24(u,n0));
		
		for(int k = 0; k < n0; k++){
		  int offSetA = offSetAPreCalc + k;
		  float2 elementA = vload2(offSetA, dftMat);
    	  
    	// ERROR HAPPENS SOMEWHERE HERE
			int offSetB = offSetBPreCalc + k;
		    float2 elementB = vload2(offSetB, x);
		    value += cmult(elementA, elementB);
		}
	     
		float pixEnergy = length(value);
    ...
    )
{
    ...
    for ( uint i = 0; i < n_per_work_item; i++, lcl_off += LOCAL_GROUP_XDIM)
    {
		...
		priv_acc += ( in_range ) ? squaredLength(vload2(lcl_off, x)) : 0.f; // accumulate result	
    }
