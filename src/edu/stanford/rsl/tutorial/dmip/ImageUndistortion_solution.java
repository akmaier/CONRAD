...
public class ImageUndistortion_solution {
	...
	public void doImageUndstortion(){
		...
		if(caseNo == 0)
		{
			filename = "C:/Users/Bier/Documents/DMIP lectures/exercises/2015/2/frame32.jpg";
		}
		else if(caseNo == 1)
		{
			filename = "C:/Users/Bier/Documents/DMIP lectures/exercises/2015/2/undistorted.jpg";
		}else if(caseNo == 2)
		{
			filename = "C:/Users/Bier/Documents/DMIP lectures/exercises/2015/2/frame90.jpg";
		}
		...
		quadraticImage.show("Cropped Input Image");
		...
		// 5. Create an artificial elliptical distortion field
		// a: spread among x-direction
		// b: spread among y-direction
		// d: (d/2) = maximal value at the radius boundary
		// NumericPointwiseOperators.subtractBy(deformation, (float) (maxR * 0.5)) : shifting the positive range to half positive/half negative range
		Grid2D deformation = new Grid2D(imSize, imSize);
		float a = 3;
		float b = 9; 
		float d = 12;
		...
		for(int i = 0; i < deformation.getWidth(); i++)
		{
			for(int j = 0; j < deformation.getHeight(); j++)
			{
				float r = (float) (d * Math.sqrt( a * Math.pow(((X.getAtIndex(i, j) - half) / imSize) , 2)  + b * Math.pow(((Y.getAtIndex(i, j) - half) / imSize) , 2) ) ) ;
				deformation.setAtIndex(i, j, r);
			}
		}
		
		float maxR = NumericPointwiseOperators.max(deformation);
		NumericPointwiseOperators.subtractBy(deformation, (float) (maxR * 0.5));
		deformation.show("Deformation field");
		...
		NumericPointwiseOperators.addBy(Xd, deformation);
		NumericPointwiseOperators.addBy(Yd, deformation);
		...
		// grid coordiates from the preprocessing part.
	...
	public static void main(String[] args) {
		...
		ImageUndistortion_solution iu = new ImageUndistortion_solution();
