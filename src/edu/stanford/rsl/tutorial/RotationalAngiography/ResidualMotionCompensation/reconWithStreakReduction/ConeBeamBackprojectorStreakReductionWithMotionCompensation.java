...
public class ConeBeamBackprojectorStreakReductionWithMotionCompensation {
	
	private int gat_ign = 3;


	public ConeBeamBackprojectorStreakReductionWithMotionCompensation() {
	...
	public Grid3D backprojectPixelDrivenCL(Grid3D sino, Grid3D transformationVectorsX, Grid3D transformationVectorsY) {
		Configuration conf = Configuration.getGlobalConfiguration();
		Trajectory geo = conf.getGeometry();
		...
		try {
			program = context.createProgram(this.getClass().getResourceAsStream("ConeBeamBackprojectorStreakReductionWithMotionCompensation.cl"))
		...
		*/
		...
		CLBuffer<FloatBuffer> transformation_y = context.createFloatBuffer(maxProjs* maxU * maxV, Mem.READ_ONLY);
		for(int p = 0; p < maxProjs; p++) {
			for(int row = 0; row < maxU; row++) { //maxU is Detector Height
				for(int col = 0; col < maxV; col++) { //maxV is Detector Width
					float val = transformationVectorsY.getAtIndex(col, row, p);
					//transformation_y.getBuffer().put(0.0f);
					transformation_y.getBuffer().put(val);
				}
			}
		}
		transformation_y.getBuffer().rewind();
		
		CLBuffer<FloatBuffer> transformation_x = context.createFloatBuffer(maxProjs* maxU * maxV, Mem.READ_ONLY);
		for(int p = 0; p < maxProjs; p++) {
			for(int row = 0; row < maxU; row++) {
				for(int col = 0; col < maxV; col++) {
					float val = transformationVectorsX.getAtIndex(col, row, p);
					//transformation_x.getBuffer().put(0.0f);
					transformation_x.getBuffer().put(val);
				}
			}
		}
		transformation_x.getBuffer().rewind();
		...
		queue.putWriteBuffer(transformation_x, true);
		queue.putWriteBuffer(transformation_y, true);
		...
		for(int p = 0; p < maxProjs; p++) {
			...
			kernel.putArg(sinoGrid).putArg(imgBuffer).putArg(destBuffer).putArg(projMatrices).putArg(transformation_x).putArg(transformation_y)
					.putArg(p)
					.putArg(maxU)
					.putArg(maxV)
					.putArg(imgSizeX).putArg(imgSizeY).putArg(imgSizeZ)
