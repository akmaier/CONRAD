...
kernel void backProjectPixelDrivenCL(image2d_t sino,global float* imgGrid,global float* destGrid,global float* gProjMatrix, global float* transformation_x,global float* transformation_y, /* not yet... float2 gridSpacing, */ int p, int detectorWidth,
	int detectorHeight,int imgSizeX,int imgSizeY,int imgSizeZ,float originX, float originY,float originZ,float spacingX,float spacingY,float spacingZ, int gat_ign);
...
kernel void backProjectPixelDrivenCL(
	...
	global float* transformation_x,
	global float* transformation_y,
	...
	//add the origin instead of subtracting it -> not a good idea
	//float4 pos = {(float)x*spacingX+originX,(float)y*spacingY +originY,0.0, 1.0};
	...
	for (int z=0;z<imgSizeZ;++z) {
		...
		//add the origin instead of subtracting 
		//pos.z = ((float)z * spacingZ) +originZ;
		...
		//falls die werte zeilenweise eingelesen werden von 0 bis 620*480
		//float deform_x = deformationGrid_x[p*y*Sizex(also width des bildes) + x]; 
		//float deform_y = deformationGrid_y[p*y*Sizex(also width des bildes) + x];
		
		//float deform_x = transformation_x[p*y*detectorWidth + x];
		//float deform_y = transformation_y[p*y*detectorWidth + x];
		
		float deform_x = transformation_x[y*detectorWidth + x + (p*detectorWidth*detectorHeight)];
		float deform_y = transformation_y[y*detectorWidth + x + (p*detectorWidth*detectorHeight)];
		...
		float fu = r* denom;
		float fv = s* denom;
		float2 posUV = {fu+0.5f + deform_x, fv+0.5f + deform_y};  //,(float)p+0.5f,0.f};
