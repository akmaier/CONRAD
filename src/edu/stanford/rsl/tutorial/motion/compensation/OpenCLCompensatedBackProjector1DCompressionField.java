 ...
 *
 */
public class OpenCLCompensatedBackProjector1DCompressionField extends VOIBasedReconstructionFilter
		implements Runnable, Citeable {
		...
		if (!initialized) {
			...
			try {
				...
				if (program == null || !program.getContext().equals(this.context)) {
					program = context.createProgram(OpenCLCompensatedBackProjector.class
							.getResourceAsStream("compensatedBackprojectCL1DCompression.cl")).build();
				}
	...
	private synchronized void projectSingleProjection(int projectionNumber, int dimz, float[] motionfield) {
		...
		kernelFunction.putArg(volumePointer).putArg(motionfieldPtr).putArg((int) lineOffset).putArg(reconDimensionZ)
	...
	private synchronized void workOnProjectionData(float[][] motionfield) {
		if (projectionsAvailable.size() > 0) {
			...
			projectSingleProjection(p, getGeometry().getReconDimensionZ(), motionfield[p]);
