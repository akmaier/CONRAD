...
public class Skeletonize
{
	...
	public Skeletonize(boolean IS_2D){
		this.IS_2D = IS_2D;
	}
	...
	public ImagePlus run(ImagePlus imp) 
	{
		...
		ImageStack skeletonized = new ImageStack(width, height);
		
		if(IS_2D){
			for(int i = 1; i <= imp.getStackSize(); i++){
				ImageStack temp = new ImageStack(width, height);
				temp.addSlice(imp.getImageStack().getProcessor(i));
				temp = runInternal(temp);
				skeletonized.addSlice(temp.getProcessor(1));
			}
		}else{
			skeletonized = runInternal(imp.getImageStack());
		}
		
		ImagePlus output = new ImagePlus();
		output.setTitle("Skeletonized Image");
		output.setStack(skeletonized);
		return output;
	} 
	
	
	private ImageStack runInternal(ImageStack input){
		this.depth = input.getSize();
		...
		for (int z = 0; z < depth; z++) 
			for (int x = 0; x < width; x++) 
				for (int y = 0; y < height; y++)
					if ( ((byte[]) outputImage.getPixels(z + 1))[x + y * width] != 0 )
