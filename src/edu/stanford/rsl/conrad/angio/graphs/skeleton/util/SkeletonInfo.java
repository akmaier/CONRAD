...
/**
 * Main class for skeleton image analysis.
 * Adapted from Ignacio Arganda-Carreras <iarganda@mit.edu> released under GNU General Public License. 
...
public class SkeletonInfo
{
	...
	private boolean IS_2D = true;
		
	public SkeletonInfo(){
	}
	
	public SkeletonInfo(boolean IS_2D){
		this.IS_2D = IS_2D;
	}
	
	public SkeletonResult[] run(ImagePlus imp){
		return run(imp, calculateShortestPath, silent, verbose);
	}
	 ...
	 * @return
	 */
	public SkeletonResult[] run(ImagePlus imp, boolean shortPath, boolean sil, boolean verb){
		this.width = imp.getWidth();
		this.height = imp.getHeight();
		SkeletonResult[] res = new SkeletonResult[(!IS_2D)?1:imp.getStackSize()];
		
		calculateShortestPath = shortPath;
		SkeletonInfo.verbose = verb;
		silent = sil;
		
		if(IS_2D){
			for(int i = 1; i <= imp.getStackSize(); i++){
				ImagePlus tImp = new ImagePlus();
				ImageStack temp = new ImageStack(width, height);
				temp.addSlice(imp.getImageStack().getProcessor(i));
				tImp.setStack(temp);
				res[i-1] = runInternal(calculateShortestPath, tImp, silent, verbose);
				if(!silent){
					showResults();
				}
			}
		}else{
			res[0] = runInternal(calculateShortestPath, imp, silent, verbose);
		}
	...
	public SkeletonResult runInternal(boolean shortPath, ImagePlus origIP, boolean silent, boolean verbose)
	{
		this.silent = silent;
		SkeletonInfo.calculateShortestPath = shortPath;
		SkeletonInfo.verbose = verbose;
	...
	public ArrayList<Point> getJunctionPoints(){
	...
	public ArrayList<ArrayList<Point>>[] getSingleJunctions(){
		return listOfSingleJunctions;
	}
	
	public ArrayList<Point> getSlabPoints(){
	 ...
	 */
	private void processSkeleton(ImageStack inputImage2) 
	{
		...
		{			
			...
			for(int i = 0; i < this.numberOfJunctions[iTree]; i ++)
			{
				...
				for(int j = 0; j < groupOfJunctions.size(); j++)
				{
					...
					for(int k = 0; k < 27; k++)
						if (neighborhood[k] == SkeletonInfo.SLAB 
							|| neighborhood[k] == SkeletonInfo.END_POINT)
							nBranch++;
				}
	...
	{		
		return getPixel(this.taggedImage, point.x, point.y, point.z) == SkeletonInfo.SLAB;
	}
	...
	{		
		return getPixel(this.taggedImage, point.x, point.y, point.z) == SkeletonInfo.JUNCTION;
	}	
	...
	{		
		return getPixel(this.taggedImage, point.x, point.y, point.z) == SkeletonInfo.END_POINT;
	}	
	...
	{		
		return getPixel(this.taggedImage, x, y, z) == SkeletonInfo.JUNCTION;
	}	
			...
			for (int x = 0; x < width; x++) 
				for (int y = 0; y < height; y++)
				{
					if(getPixel(inputImage2, x, y, z) != 0)
					{
						...
						if(numOfNeighbors < 2)
						{
							setPixel(outputImage, x, y, z, SkeletonInfo.END_POINT);
						...
						{
							setPixel(outputImage, x, y, z, SkeletonInfo.JUNCTION);
						...
						{
							setPixel(outputImage, x, y, z, SkeletonInfo.SLAB);
