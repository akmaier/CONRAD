...
public abstract class ImageUtil {
	...
	public static Grid3D wrapImagePlus(ImagePlus image, boolean copy, boolean invertStack) {
		...
		if (image.isHyperStack()) {
			if (invertStack)
				throw new RuntimeException("Stack inversion is not implemented for hyperstacks yet. Sorry");
			...
			for (int i = 0; i < dimz; i++) {
				MultiChannelGrid2D multiChannelGrid2D = new MultiChannelGrid2D(image.getWidth(), image.getHeight(),
						channels);
			...
			for (int i = 0; i < image.getDimensions()[3]; i++) {
				int pos = i;
				if (invertStack)
					pos = image.getDimensions()[3] - 1 - i;
				revan.setSubGrid(pos, wrapImagePlusSlice(image, i + 1, copy));
			}
		}
		revan.setSpacing(image.getCalibration().pixelWidth, image.getCalibration().pixelHeight,
		...
		// System.out.println(image.getCalibration().pixelWidth + " " +
	...
	public static void applyConradImageCalibration(ImagePlus image, boolean isReconstruction) {
		...
		// FIXME: If we have a projection domain output the x and y spacing and origin
		// refer to the detector
		// In case of a reconstruction output this dimensions should be equal to the
		// global settings!
		// FIXME: Find a nicer way to handle difference between projection and
	...
	public static Grid2D wrapImageProcessor(ImageProcessor ip, boolean copy) {
		if (ip instanceof FloatProcessor) {
			if (!copy)
				...
				return wrapFloatProcessor((FloatProcessor) ip.duplicate());
		} else {
	...
	/**
	 * Increases the pixel values of all pixels in the ImagePlus by addition of
	 * value
	 * 
	 * @param image the ImagePlus
	...
	/**
	 * Normalizes all pixel values of an ImagePlus to mean 0 and standard deviation
	 * 1.
	 * 
	 * @param image the imagePlus
	 * @return an array with two entries. The first one is the mean of all pixel
	 *         values. The second one the standard deviation of all pixel values.
	 */
	public static double[] normalizeImagePlusCutOff(ImagePlus image, int numStandardDeviations) {
		if (image.getStackSize() == 1) {
			...
			for (int i = 0; i < image.getStackSize(); i++) {
				double[] temp = ImageUtil.normalizeImageProcessorCutOff(image.getStack().getProcessor(i + 1),
	 ...
	 * 
	 * @param image the imagePlus
	 * @return an array with two times stack size entries. The respective first one
	 *         is the mean of all pixel values. The second one the standard
	 *         deviation of all pixel values.
	 */
	 ...
	 * @return an double array with two entries. The first one is the mean and the
	 *         second one is the standard deviation.
	 */
	...
	/**
	 * Normalizes an ImageProcessor to mean 0 and standard deviation 1. Cuts off
	 * values greater than a certain amount of standard deviations.
	 * 
	 * @param imp                   the image processor
	 * @param numStandardDeviations Number of standard deviations after which the
	 *                              values are cut off
	 * @return an double array with two entries. The first one is the mean and the
	 *         second one is the standard deviation.
	 */
		...
		for (int i = 0; i < information.getWidth(); i++) {
			for (int j = 0; j < information.getHeight(); j++) {
				...
				if (nominator.getChannelProcessor().getPixelValue(i, j) != 0) {
					value = nominator.getChannelProcessor().getPixelValue(i, j)
							/ denominator.getChannelProcessor().getPixelValue(i, j);
				}
		...
		for (int i = 0; i < information.getWidth(); i++) {
			for (int j = 0; j < information.getHeight(); j++) {
				...
				if (nominator.getChannelProcessor().getPixelValue(i, j) != Float.NaN) {
					value = nominator.getStack().getProcessor(n).getPixelValue(i, j)
							* denominator.getStack().getProcessor(n).getPixelValue(i, j);
				}
				information.putPixelValue(i, j, value);
			}
		}
	...
	/**
	 * Divides two image processors in the two given ImagePlus. Will allocate a new
	 * FloatProcessor for the result.
	 * 
	...
	public static FloatProcessor divideImages(ImagePlus nominator, ImagePlus denominator, int n) {
		...
		for (int i = 0; i < information.getWidth(); i++) {
			for (int j = 0; j < information.getHeight(); j++) {
				double value = Double.MAX_VALUE;
				if (nominator.getChannelProcessor().getPixelValue(i, j) != 0) {
					value = nominator.getStack().getProcessor(n + 1).getPixelValue(i, j)
							/ denominator.getStack().getProcessor(n + 1).getPixelValue(i, j);
				}
				information.putPixelValue(i, j, value);
			}
		}
	...
	/**
	 * Subtracts two image processors. Works in place. First ImageProcessor is
	 * modified.
	 * 
	 * @param left  the first processor
	 * @param right the second processor
	...
	/**
	 * Creates a two-dimensional symmetric gaussian filter kernel equal to the
	 * Matlab method
	 * 
	...
	public static Grid2D create2DGauss(int sizeX, int sizeY, double sigma) {
		...
		for (int y = 0; y < out.getHeight(); y++) {
			for (int x = 0; x < out.getWidth(); x++) {
				...
				// out.setAtIndex(x, y, (float)(((double)size) *
				// Math.exp(-0.5*(xx*xx+yy*yy)/sigma/sigma)/(2*Math.PI*Math.sqrt(2)*sigma)));
		...
		/*
		 * maxEps *= 1.1921e-07; for (int y = 0; y < out.getHeight(); y++) { for (int x
		 * = 0; x < out.getWidth(); x++) { if (out.getAtIndex(x, y) > maxEps)
		 * out.setAtIndex(x, y, out.getAtIndex(x, y)/sum); else out.setAtIndex(x,y,0); }
		 * }
		 */
