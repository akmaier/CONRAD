...
public class HandleExtraFileTypes extends ImagePlus implements PlugIn {
	...
	private Object tryOpen(String directory, String name, String path) {
		...
		// Bio formats NRRD Reads
		// Open NRRD files
		if (name.endsWith(".nrrd") || name.endsWith(".nhdr")) {
			return tryPlugIn("Nrrd_Reader", path);
		}
		// Viva file handler
		if (name.endsWith(".viv") || name.endsWith(".seq")) {
			return tryPlugIn("Viva_Reader", path);
		}
		
		// Viva file handler
		if (name.endsWith(".mkt")) {
			return tryPlugIn("MKT_Reader", path);
		}
		// Dennerlein file handler
		if (name.endsWith(".bin")) {
			if (name.matches(".*\\dx\\d.*")){
				return tryPlugIn("RCL_Reader", path);
			}
			return tryPlugIn("Dennerlein_Reader", path);
		}
		// Nurbs Reader for XCat's nrb format
		if (name.endsWith(".nrb")) {
			return tryPlugIn("Nurbs_Reader", path);
		}
		// Nurbs Reader for XCat's nrb format
		...
		if (o==null && (IJ.getVersion().compareTo("1.38j")<0||!IJ.redirectingErrorMessages()) && (new File(path).exists())) {
			...
			if (loci!=null) {
				...
				try {
					...
					Class<? extends Object> c = loci.getClass();
